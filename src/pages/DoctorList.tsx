...
import { Search, Filter, MapPin, Stethoscope, User as UserIcon, Download, Plus, X, ArrowRight, Building2, Briefcase, Trash2, Upload, FileText, AlertCircle } from 'lucide-react';

interface DoctorListProps {
  doctors: Doctor[];
  onAddDoctor?: (doc: Doctor) => void;
  onAddDoctors?: (docs: Doctor[]) => void;
...
const DoctorList: React.FC<DoctorListProps> = ({ doctors, onAddDoctor, onAddDoctors, onDeleteDoctor, user }) => {
  ...
  const handleExport = () => {
    const headers = [
        "Categoria", "Nombre", "Ejecutivo", "Especialidad/Area", "Subespecialidad", "Hospital", 
        "Dirección", "Telefono", "Email", "Cedula", "Notas"
    ];
  ...
  const processCSV = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      reader.onload = (event) => {
          try {
              ...
              // Split rows handling potentially different line endings
              const rows = text.split(/\r\n|\n/);
              
              if (rows.length < 2) {
                  alert("El archivo CSV parece estar vacío o solo contiene encabezados.");
                  return;
              }
              ...
              // Helper to find column index roughly matching name
              const findIdx = (keywords: string[]) => headers.findIndex(h => keywords.some(k => h.includes(k)));

              // Mapping indexes
              const idxName = findIdx(['NOMBRE', 'DOCTOR', 'ADMINISTRATIVO', 'HOSPITAL']);
              const idxExec = findIdx(['EJECUTIVO', 'ASIGNADO']);
              const idxSpec = findIdx(['ESPECIALIDAD', 'AREA', 'PUESTO']);
              const idxSubSpec = findIdx(['SUBESPECIALIDAD']);
              const idxHosp = findIdx(['HOSPITAL', 'CLINICA', 'UBICACION']);
              const idxAddr = findIdx(['DIRECCION', 'CALLE', 'UBICACIÓN']);
              const idxPhone = findIdx(['TELEFONO', 'CELULAR', 'MOVIL']);
              const idxEmail = findIdx(['EMAIL', 'CORREO']);
              const idxCedula = findIdx(['CEDULA', 'CÉDULA']);
              const idxNotes = findIdx(['NOTA', 'OBSERVACION']);

              if (idxName === -1) {
                  alert("No se encontró la columna de NOMBRE en el CSV.");
                  return;
              }
              ...
              for (let i = 1; i < rows.length; i++) {
                  ...
                  // Complex split to handle commas inside quotes "Like, This"
                  ...
                  const newDoc: Doctor = {
                      id: `${activeTab.substring(0,3).toLowerCase()}-imp-${Date.now()}-${i}`,
                      category: activeTab,
                      name: name,
                      executive: (idxExec > -1 ? cols[idxExec] : user.name).toUpperCase() || 'SIN ASIGNAR',
                      specialty: (idxSpec > -1 ? cols[idxSpec] : (activeTab === 'HOSPITAL' ? 'HOSPITAL' : '')).toUpperCase(),
                      subSpecialty: (idxSubSpec > -1 ? cols[idxSubSpec] : '').toUpperCase(),
                      address: (idxAddr > -1 ? cols[idxAddr] : '').toUpperCase(),
                      hospital: (idxHosp > -1 ? cols[idxHosp] : '').toUpperCase(),
                      area: (idxSpec > -1 ? cols[idxSpec] : '').toUpperCase(), // Map Area to Specialty column if admin
                      phone: (idxPhone > -1 ? cols[idxPhone] : ''),
                      email: (idxEmail > -1 ? cols[idxEmail] : ''),
                      cedula: (idxCedula > -1 ? cols[idxCedula] : ''),
                      importantNotes: (idxNotes > -1 ? cols[idxNotes] : '').toUpperCase(),
                      visits: [],
              ...
              if (newDocs.length > 0) {
                  if (onAddDoctors) {
                      onAddDoctors(newDocs);
                  } else if (onAddDoctor) {
                      // Fallback one by one
                      newDocs.forEach(d => onAddDoctor(d));
                  }
                  alert(`Se han importado ${newDocs.length} registros exitosamente a la sección ${activeTab}.`);
              } else {
  ...
  const handleAddSubmit = (e: React.FormEvent) => {
      ...
      resetForm();
      // Feedback visual sutil en lugar de alert intrusivo
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
            ...
            {user.role === 'admin' && (
                <>
                    <button 
                        ...
                        className="flex items-center px-4 py-2.5 bg-emerald-600 text-white rounded-xl hover:bg-emerald-700 transition shadow-sm font-bold text-sm active:scale-95"
                        title="Importar CSV (Nombre, Dirección, Teléfono, etc.)"
                    >
            ...
            <button 
                ...
                className="flex items-center px-4 py-2.5 bg-white border border-slate-200 text-slate-600 rounded-xl hover:bg-slate-50 transition shadow-sm font-bold text-sm"
            >
