...
const App: React.FC = () => {
  ...
  const importFullBackup = useCallback((data: { doctors: Doctor[], procedures: Procedure[], timeOff?: TimeOffEvent[] }) => {
  ...
  const updateDoctorsList = useCallback((newDoctors: Doctor[]) => {
  ...
  // Bulk Import Function for CSV
  const addDoctors = useCallback((newDoctors: Doctor[]) => {
  ...
  const handleDeleteVisit = useCallback((doctorId: string, visitId: string) => {
  ...
  const addProcedure = useCallback((newProc: Procedure) => {
      setProcedures(prev => [...prev, newProc]);
  }, []);

  const updateProcedure = useCallback((updatedProc: Procedure) => {
        ...
        <div className={`flex-1 flex flex-col h-full relative pt-16 md:pt-0 transition-all duration-300 ease-in-out ${isSidebarCollapsed ? 'md:ml-20' : 'md:ml-64'}`}>
          <main className="flex-1 overflow-x-auto overflow-y-auto p-4 md:p-8 relative z-10 w-full">
            <div className="max-w-7xl mx-auto min-w-[320px]">
                ...
                <Route path="/doctors" element={<DoctorList doctors={doctors} onAddDoctor={addDoctor} onAddDoctors={addDoctors} onDeleteDoctor={deleteDoctor} user={user} />} />
